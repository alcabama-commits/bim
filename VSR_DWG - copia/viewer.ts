import { DXFParser, DXFDrawing, DXFEntity } from './services/dxfParser.js';

class DXFViewer {
    private canvas: HTMLCanvasElement;
    private ctx: CanvasRenderingContext2D;
    private drawing: DXFDrawing | null = null;
    ...
    loadDrawing(drawing: DXFDrawing): void {
    ...
    private renderEntity(entity: DXFEntity): void {
    ...
    private renderLine(entity: DXFEntity): void {
    ...
    private renderCircle(entity: DXFEntity): void {
        if (entity.x1 === undefined || entity.y1 === undefined ||
    ...
    private renderArc(entity: DXFEntity): void {
        if (entity.x1 === undefined || entity.y1 === undefined ||
            entity.radius === undefined) {
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadFile(file: File): void {
        if (statusText) statusText.textContent = 'Cargando archivo...';
        ...
        reader.onload = (e) => {
            try {
                ...
                if (entityCount) entityCount.textContent = `${drawing.entities.length} entidades`;
                if (statusText) statusText.textContent = 'Archivo cargado correctamente';
                
                // Scroll a la sección del visor
                viewerSection?.scrollIntoView({ behavior: 'smooth' });
            } catch (error) {
                console.error('Error parsing DXF:', error);
                if (statusText) statusText.textContent = 'Error al cargar el archivo DXF';
                alert('Error al cargar el archivo DXF. Por favor, verifica que el archivo sea válido.');
            }
        };
        
        reader.onerror = () => {
            if (statusText) statusText.textContent = 'Error al leer el archivo';
