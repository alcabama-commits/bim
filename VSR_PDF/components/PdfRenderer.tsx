...
const PdfRenderer: React.FC<PdfRendererProps> = ({ 
  ...
  if (!file) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-[#605E62] border-2 border-dashed border-[#827E84] m-8 rounded-3xl">
        <div className="text-center space-y-4 max-w-sm p-8">
          <div className="w-20 h-20 bg-[rgba(211,4,92,0.1)] rounded-full flex items-center justify-center mx-auto mb-6 border border-[rgba(211,4,92,0.2)]">
          ...
          <p className="text-[#A49FA6] text-sm">Arrastra tu archivo o selecciónalo para iniciar el visor métrico.</p>
          <label className="inline-block cursor-pointer bg-[#D3045C] hover:bg-[#D3045C] text-[#FFFFFF] px-8 py-3 rounded-xl font-bold transition-all transform hover:scale-105 active:scale-95 shadow-[0_0_25px_rgba(211,4,92,0.4)]">
  ...
  return (
    <div 
      ...
      onWheel={handleWheel}
      className={`relative flex-1 overflow-auto bg-[#605E62] h-full no-scrollbar touch-none ${tool === 'hand' ? (isDragging ? 'cursor-grabbing' : 'cursor-grab') : 'cursor-crosshair'}`}
    >
      <div className="relative w-fit min-w-full min-h-full flex p-20">
        <div className={`relative transition-all duration-300 m-auto ${isBlueprint ? 'invert hue-rotate-180 brightness-110 contrast-125' : ''}`}>
          <canvas ref={canvasRef} className="bg-white shadow-[0_0_60px_rgba(0,0,0,0.6)] border border-[#827E84]" />
          ...
          <svg className="absolute inset-0 pointer-events-none w-full h-full">
            {points.map((p, i) => (
              <circle key={i} cx={p.x} cy={p.y} r="6" fill="#D3045C" stroke="#000000" strokeWidth="2" />
            ))}
            {points.length === 2 && (
              <>
                ...
                <g transform={`translate(${(points[0].x + points[1].x) / 2}, ${(points[0].y + points[1].y) / 2 - 20})`}>
                  <rect x="-50" y="-12" width="100" height="24" rx="12" fill="#000000" stroke="#D3045C" strokeWidth="2" />
      ...
      {loading && (
        <div className="absolute inset-0 bg-[rgba(0,0,0,0.8)] backdrop-blur-md flex items-center justify-center z-50">
          <div className="flex flex-col items-center gap-6">
            <div className="w-16 h-16 border-4 border-[rgba(211,4,92,0.2)] border-t-[#D3045C] animate-spin rounded-full"></div>
            <div className="text-center">
              <span className="block text-[#D3045C] font-mono text-xs tracking-widest uppercase mb-1">Cargando Render BIM</span>
              <span className="text-[#827E84] text-[10px] uppercase font-bold tracking-widest">Calculando vectores...</span>
            </div>
          </div>
        </div>
      )}
