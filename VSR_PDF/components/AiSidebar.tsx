    ...
    <div className="w-96 border-l border-slate-200 bg-white flex flex-col h-full shadow-xl">
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-slate-100 p-1 rounded-lg">
            <button 
              ...
              className={`px-3 py-1 text-sm rounded-md transition ${activeTab === 'chat' ? 'bg-white shadow-sm text-[#D3045C] font-medium' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Chat
            </button>
            <button 
              onClick={() => setActiveTab('summary')}
              className={`px-3 py-1 text-sm rounded-md transition ${activeTab === 'summary' ? 'bg-white shadow-sm text-[#D3045C] font-medium' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Resumen
            </button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar" ref={scrollRef}>
        {!isPdfLoaded ? (
          ...
          <>
            {messages.length === 0 && (
              <div className="bg-[#D3045C]/10 border border-[#D3045C]/20 rounded-xl p-4 text-sm text-[#D3045C]">
            ...
            {!summary ? (
              <div className="flex flex-col items-center justify-center py-10">
                <button 
                  ...
                  className="bg-[#D3045C] text-white px-6 py-2 rounded-full text-sm font-medium hover:bg-[#c30352] transition flex items-center gap-2"
                >
        ...
        <div className="p-4 border-t border-slate-100">
          <form onSubmit={handleSendMessage} className="relative">
            <input 
              ...
              className="w-full pl-4 pr-12 py-3 bg-slate-100 border-none rounded-xl text-sm focus:ring-2 focus:ring-[#D3045C] outline-none disabled:opacity-50 transition-all"
            />
            <button 
              type="submit"
              disabled={!isPdfLoaded || isLoading || !input.trim()}
              className="absolute right-2 top-1.5 w-9 h-9 bg-[#D3045C] text-white rounded-lg flex items-center justify-center hover:bg-[#c30352] disabled:opacity-50 transition-colors"
            >
