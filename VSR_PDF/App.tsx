...
const App: React.FC = () => {
  ...
  useEffect(() => {
    console.log("%c ALCABAMA BIM v2.2 CARGADO ", "background: #D3045C; color: #fff; font-size: 20px; padding: 10px;");
  }, []);
        ...
        <header className="h-12 bg-[#000000] border-b border-[#605E62] px-4 flex items-center justify-between z-30 shadow-md">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold tracking-tighter uppercase">ArchView <span className="text-[#D3045C] text-[10px] ml-1">BIM PRO v2.2</span></span>
            </div>
          ...
          <div className="flex items-center gap-1">
            <div className="flex bg-[#605E62]/30 rounded p-0.5 border border-[#605E62] mr-4">
              ...
              <button 
                ...
                className={`w-8 h-8 flex items-center justify-center rounded transition ${activeTool === 'calibrate' ? 'bg-[#D3045C] shadow-inner text-white' : 'hover:bg-[#605E62]'}`}
            ...
            <button onClick={() => setShowGrid(!showGrid)} className={`w-8 h-8 rounded transition ${showGrid ? 'text-[#D3045C] bg-[#D3045C]/10' : 'text-[#827E84] hover:bg-[#605E62]'}`} title="Grid"><i className="fa-solid fa-border-none text-xs"></i></button>
            <button onClick={() => setIsBlueprint(!isBlueprint)} className={`w-8 h-8 rounded transition ${isBlueprint ? 'text-[#D3045C] bg-[#D3045C]/10' : 'text-[#827E84] hover:bg-[#605E62]'}`} title="Modo Blueprint"><i className="fa-solid fa-eye-slash text-xs"></i></button>
          </div>
